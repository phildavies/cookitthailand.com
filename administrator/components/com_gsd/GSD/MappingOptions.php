...
class MappingOptions
{
    ...
    /**
     * Recursively convert an array to an object (deep conversion)
     *
    ...
    private static function arrayToObject($data)
    {
        if (is_array($data))
        {
            return (object) array_map([self::class, 'arrayToObject'], $data);
        }
    ...
    public static function prepare(&$properties)
    {
        ...
        {
            /**
             * In Joomla 6, Registry's Iterator returns arrays instead of objects when iterating.
             * 
             * We need to convert arrays to objects during iteration.
             */
            if (is_array($property))
            {
                $property = self::arrayToObject($property);
            }
