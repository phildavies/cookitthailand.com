...
class File 
{
	...
	{
		// Reject moves whose source escapes JPATH_ROOT/tmp_path.
		if (!self::isWithinAllowedRoots(Path::clean($source_file)))
		{
	 ...
	 * Delete a file.
	 *
	 * Accepts either an absolute path or a path relative to JPATH_ROOT. The
	 * resolved path must sit inside an allowed root — see isWithinAllowedRoots().
	 *
	 * @param   string  $filename	The absolute or JPATH_ROOT-relative file path
	 *
	 * @return  bool
	 */
	public static function delete($filename)
	{
		...
		if (\Tassos\Framework\Security\SafePath::isAbsolute($filename))
		{
			$file = Path::clean($filename);
		}
		else
		{
			// Defense-in-depth: reject traversal sequences on relative input
			try
			{
				\Tassos\Framework\Security\SafePath::sanitize($filename);
			}
			catch (\Throwable $e)
			{
				return false;
		...
		if (!self::isWithinAllowedRoots($file))
		{
			return false;
		}
	...
	/**
	 * True if $file sits inside JPATH_ROOT, or inside Joomla's configured
	 * tmp_path (so deployments with tmp_path outside the web root can still
	 * clean up uploads).
	 *
	 * @param	string	$file	An absolute, Path::clean()-ed file path
	 *
	 * @return	bool
	 */
	private static function isWithinAllowedRoots($file)
	{
		if (\Tassos\Framework\Security\SafePath::isWithin($file, JPATH_ROOT))
		{
			return true;
		}
		...
		if (is_string($tmp_path) && $tmp_path !== '' && is_dir($tmp_path)
			&& \Tassos\Framework\Security\SafePath::isWithin($file, $tmp_path))
		{
			return true;
		}
